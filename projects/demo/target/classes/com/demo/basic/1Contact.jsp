...
var layoutSingleRow=[{
 ...
 //field????????¨°???js??toArray¡Á???querydate??
				 ...
				 cells: [ [
      				  ...
      				  {name: i18nStr.codeContact, field: 2,width: 8}
...
function querybutton(){
	...
	if(dojo.byId("querycustomerForContactDiv")!=undefined && getQueryCondcustomerForContact(customerQuery)){
...
function getVO(queryvo,pagenumber,pagesize,orderBy){
      ...
      SOAPClient.invoke(url,"getContact", pu, false,  function GetEnu_callBack(ul, soapResponse){      
           querydata=new Array();
		   querydata=dojoContact.ContactVOtoArray_fun(ul,false);
...
function addDivSave(){
	...
	var passAll=0;
										//enumeration ?¡§String??¨ª??
				if(dijit.byId("addcodeContact").validate(true)){
					vobject.codeContact=dijit.byId("addcodeContact").getValue();
				}else{
					setErrorClass(dijit.byId("addcodeContact")); passAll=1;
				}
...
//set ($inume =1) -->
					...
					<td align="left" ><!--colspan="3" --><input  id="addcustomer"  jsId="addcustomer" class="editerInput" dojoType="dijit.form.myFilteringSelect" store="dojoContact.customerStore" onchange="customerSet(0,arguments[0]);" required="true" /></td>  
...
//set ($inume =1) -->
					...
					<td align="left" colspan="3"  ><!----><table class="innerTable">
							<tr>
								<td   ><input  id="querycustomer"  jsId="querycustomer" class="selectInput" dojoType="dijit.form.myFilteringSelect" store="dojoContact.customerStore"  /></td><td align="left" ><div dojoType="dijit.form.myDropDownButton" iconClass="dialogIcon" class="downButtonStyle"  ><div dojoType="dijit.TooltipDialog"  href="../common/querycustomerForContactFloat.html"title="detail" execute="" ></div></div>
					</td>
							</tr>
						</table></td>  
