...
<title></title>
	...
	//
				...
				{ name: '\u8be6\u7ec6', colSpan: 11 ,styles: 'padding: 0; margin: 0;', get: getDetailProd }//value: '<div style="height:0px; background-color: white;"></div>'
			  ]
		  ]
...
function getDetailProd(inRowIndex){
	if(detailRowsProd[inRowIndex]){
		var cell = this;
		setTimeout(function() { buildSubgridProd(inRowIndex, cell); }, 0);  }
		var h =0;
		if(detailRowsProd[inRowIndex]&&querydataProd[inRowIndex][$childRow]!=undefined &&querydataProd[inRowIndex][$childRow].length>0){
...
var addRowProd = function(){
	gridProd.addRow([false,0,"","",""],gridProd.model.getRowCount());
} 
...
var  d=new DateTimeQuery();
       ...
       if(passAll==0){
             if(thisCell[1]==0){//create new row
				...
				querydataProd[i][${childRow}]=[addrow];
				});
	         }else{//modify row
                padd.add('ProdVO',vobject);
