	...
	//
		  ...
		  //(15 > 8)
			//delete2
				...
				{ name: '\u8be6\u7ec6', colSpan: 8 ,styles: 'padding: 0; margin: 0;', get: getDetailOrderRowd }//value: '<div style="height:0px; background-color: white;"></div>'
			  ]
		  ]
...
function onBeforeRow(inDataIndex, inRow) {	if (inRow[2]!=undefined)inRow[2].hidden = !detailRowsOrderRowd[inDataIndex];}	
...
function getDetailOrderRowd(inRowIndex){
	if(detailRowsOrderRowd[inRowIndex]){
		var cell = this;
		setTimeout(function() { buildSubgridOrderRowd(inRowIndex, cell); }, 0);  }
		var h =0;
		if(detailRowsOrderRowd[inRowIndex]&&querydataOrderRowd[inRowIndex][$childRow]!=undefined &&querydataOrderRowd[inRowIndex][$childRow].length>0){
...
var addRowOrderRowd = function(){
	gridOrderRowd.addRow([false,0,"","",""],gridOrderRowd.model.getRowCount());
} 
...
function call_funUpdOrderRowd() {
       ...
       if(passAll==0){
             if(thisCell[1]==0){//create new row
				...
				querydataOrderRowd[i][${childRow}]=[addrow];
				});
	         }else{//modify row
                padd.add('OrderRowdVO',vobject);
