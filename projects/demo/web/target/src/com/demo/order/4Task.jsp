...
<title></title>
	...
	//??¡è???????????????¨¨?¡§		
     ...
     <script type="text/javascript" src="../js/myi18n.js"></script>
 			...
 			<script type="text/javascript" src="../order/orderRowTasks2taskCrudAFTab.js"></script>
...
//??????layout(org.andromda.cartridges.bpm4struts.metafacades.StrutsManageableEntityAssociationEndLogicImpl[taskForm] in [org.andromda.cartridges.bpm4struts.metafacades.StrutsManageableEntityAssociationEndLogicImpl[taskForm]])if(!false)  ????Float??¨ª??¨ª¡ã???
...
function gridResize(){
			if(gridorderRowTasks2task!=undefined)gridorderRowTasks2task.resize({h: dijit.byId("bottomPane").domNode.clientHeight-90});
	 	 		gridTask.resize({h:document.body.clientHeight-dijit.byId("bottomPane").domNode.clientHeight-36});
}		
function init(){
...
setTimeout(function(){
	gridTask.resize({h: document.body.clientHeight-86});
		...
		if(connecthandle==0){
			handle=dojo.connect(dijit.byId("gridTask"), "doclick", function(e){
				...
				mainName = querydataTask[e.rowIndex][2];
				resetNU();
				clickRowTab();		
			});
			connecthandle=1;
...
<table  width="100%">
	<tr height="25px" width="100%">
       ...
       <div id="gridTask"  dojoType="dojox.Grid" autoWidth="false"  jsId="gridTask"	model="modelTask"  structure="layoutSingleRowQuery"  ></div>
    </div>  
