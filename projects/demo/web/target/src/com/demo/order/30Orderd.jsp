...
<title></title>
	...
	//å¤æ­æ¯å¦æ¯ä¸»è¡¨		
	 		...
	 		<script type="text/javascript" src="OrderRowdSelectPane.js"></script><!--Ò»Òª-->
	 		<script type="text/javascript" src="CustomerSelectPane.js"></script><!--Ò»Òª-->
	 	 					<script type="text/javascript" src="../order/OrderRowdSubCrudFor30.js"></script><!--sub Table script    eg:taskRow.js    Ð¶Ç·Îªí£¬Ò³Ê¼Ó±childRow=0-->
							<script type="text/javascript" >
...
function getDetailOrderd(inRowIndex){
	if(detailRowsOrderd[inRowIndex]){
		...
		var h =0;
		if(detailRowsOrderd[inRowIndex]&&querydataOrderd[inRowIndex][19].length>0){
...
function init(){
	...
	getOrderdVO(queryvoOrderd);
		...
		dojo.connect(dijit.byId("OrderdSelectTab"), "selectChild", function(e){
		
				dojo.byId("customer2orderdsTable").style.visibility="hidden";
										dojo.byId("orderRowds2productTable").style.visibility="hidden";
			dojo.byId(dijit.byId("OrderdSelectTab").selectedChildWidget.id+"Table").style.visibility="visible";
if(dijit.byId("gridpane"+dijit.byId("OrderdSelectTab").selectedChildWidget.id)!=undefined)dijit.byId("gridpane"+dijit.byId("OrderdSelectTab").selectedChildWidget.id).resize({h: dijit.byId("bottomPane").domNode.clientHeight-90});
  });	

  setTimeout(function(){
	gridOrderd.resize({h: dojo.byId("gridOrderd").parentNode.clientHeight-36});
	dojo.connect(dijit.byId("bottomPane"), "_showEnd", function(e){gridResize();});
	dojo.connect(dijit.byId("bottomPane"), "_hideEnd", function(e){gridResize();});	
	dojo.connect(dijit.byId("bottomPane"), "_afterResize", function(e){gridResize();}); 
  },0);
 ...
 dojo.addOnLoad(init);   
function gridResize(){
	OrderdSelectTab.resize({h: dijit.byId("bottomPane").domNode.clientHeight-40});
	if(dijit.byId("gridpane"+dijit.byId("OrderdSelectTab").selectedChildWidget.id)!=undefined)dijit.byId("gridpane"+dijit.byId("OrderdSelectTab").selectedChildWidget.id).resize({h: dijit.byId("bottomPane").domNode.clientHeight-90});
	/*								if(dijit.byId("gridpanecustomer2orderds")!=undefined)dijit.byId("gridpanecustomer2orderds").resize({h: dijit.byId("bottomPane").domNode.clientHeight-90});
																				if(dijit.byId("gridpaneorderRowds2product")!=undefined)dijit.byId("gridpaneorderRowds2product").resize({h: dijit.byId("bottomPane").domNode.clientHeight-90});
							*/
...
var addrow=[];
					...
					addrow.push('');
																					addrow.push('');
																							addrow.push('');
												if(ifMultPanecustomer2orderds!=undefined){
					var re=selectcustomer2orderdsFunClick();
					addrow.push(re);}
						addrow.splice(0,0,false,0);
	gridOrderd.addRow(addrow,gridOrderd.model.getRowCount());
} 
...
<div id="queryDivOrderd" style="margin-right: 0px;  height: 200px; display:none;" >
	...
	<td style="{width:22%}"></td></tr>  
                <tr>
						...
						<td align="left"  >
													<input  id="queryOrderdconveyanceEnum" jsId="queryOrderdconveyanceEnum" class="editerInput"  dojoType="dijit.form.myFilteringSelect" store="dojoOrderd.conveyanceEnumStore" />
												</td>  
										  </tr>
			                <tr>
						...
						<td align="left"  >
														...
														<input  id="queryOrderdcarryTypeEnum"  jsId="queryOrderdcarryTypeEnum" class="editerInput"  dojoType="dijit.form.myFilteringSelect" store="dojoOrderd.carryTypeEnumStore" />
													</td>
						...
						<td align="left"  >
														...
														<input  id="queryOrderdlevelEnum"  jsId="queryOrderdlevelEnum" class="editerInput"  dojoType="dijit.form.myFilteringSelect" store="dojoOrderd.levelEnumStore" />
													</td>
														<td align="right"  i18n="customerTypeEnum" >customerTypeEnum:</td>
						<td align="left"  colspan="3" >
												<input  id="queryOrderdcustomerTypeEnum" jsId="queryOrderdcustomerTypeEnum" class="editerInput"  dojoType="dijit.form.myFilteringSelect" store="dojoOrderd.customerTypeEnumStore" />
												</td>  
											<td align="right"  > </td>
						<td align="right"  ></td>
					</tr><tr >
...
//
					...
					<td align="left" ><input  id="queryOrderdcustomer"  jsId="queryOrderdcustomer" class="editerInput" dojoType="dijit.form.myFilteringSelect" store="dojoOrderd.customerStore"  /></td>  
						    </tr>
...
<div dojoType="dojox.layout.ExpandoPane" splitter="true" duration="125" region="bottom" title="" style="height:375px;"   id="bottomPane" jsId="bottomPane" >
											<table id="customer2orderdsTable"  style=" position:absolute;top:0px; width:90%; visibility:visible ">
				<tr height="25px" width="100%">
		<td align="right" >
					<span id="rowCountpanecustomer2orderds"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<button dojoType="dijit.form.myButton" onclick="upPagepanecustomer2orderds();"  id="but6panecustomer2orderds" disabled='true' jsId="but6panecustomer2orderds" iconClass="upPageIcon" >pageup</button>
					<button dojoType="dijit.form.myButton" onclick="downPagepanecustomer2orderds();" id="but7panecustomer2orderds" disabled='true' jsId="but7panecustomer2orderds" iconClass="downPageIcon" >pagedown</button>
	</td></tr></table>
	...
	<div dojoType="dijit.layout.TabContainer" style="height:100%; width:100%;"   id="OrderdSelectTab" jsId="OrderdSelectTab"  >
				<div dojoType="dojox.layout.ContentPane" href="../common/customer2orderdsSelectPane.html" title="customer"  id="customer2orderds" jsId="customer2orderds" >
		</div>
				<!--{member.type}====org.andromda.cartridges.bpm4struts.metafacades.StrutsManageableEntityLogicImpl[OrderRowd]-->
																										<div dojoType="dojox.layout.ContentPane" href="../common/orderRowds2productSelectPane.html" title="product"  id="orderRowds2product" jsId="orderRowds2product" ></div>
												</div>
</div>
