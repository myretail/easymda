...
//set (= order2orderRow)
	//name =order2orderRowAFTab.js  manageable.name=Orderd
...
//----------move from 30.1 begin-----------
  ...
  //rowinum=8
	...
	//
		  ...
		  //(17 > 9)
			//delete2
								 ,   {name: i18nStr.customer,field: 17,get:function(inRowIndex){ var str='';var strid;if(querydataorder2orderRow[inRowIndex]!=undefined && querydataorder2orderRow[inRowIndex][17]!=undefined && querydataorder2orderRow[inRowIndex][17][1]!=undefined){str=querydataorder2orderRow[inRowIndex][17][2];strid=querydataorder2orderRow[inRowIndex][17][1];} return str;} ,width: 10 }
...
function getDetailOrderd(inRowIndex){
	if(detailRowsOrderd[inRowIndex]){
		...
		var h =0;
		if(detailRowsOrderd[inRowIndex]&&querydataorder2orderRow[inRowIndex][16].length>0){
							h=65+25*querydataorder2orderRow[inRowIndex][16].length;  		}
...
function buildSubgridorder2orderRow(inRowIndex, inCell) {  //makeSubgridIdMainDetail(id)?idmainDetailGridid
	if(inCell!=undefined && inCell.getNode(inRowIndex)!=undefined){
		...
		var dataChild=querydataorder2orderRow[inRowIndex][16];
...
//ÅÐ¶Ï±¾±íÊÇ·ñÎª×Ó±í,µ±Ò»¸ö±íÊÇ¶à¸ö±íµÄ×Ó±íÊ±£¬´Ë´¦¿ÉÄÜÓÐ´íÎó¡£	
	querydataorder2orderRow=dojoOrderd.OrderdVOtoArray_fun(ul,false);
...
function saveSubRow(subGrid,thisCell,i){		 
	     ...
	     vobject.id=thisCell[1];
			...
			mainTable.id=querydataorder2orderRow[subGrid.dataRow][1];
            vobject.order=mainTable;
																var manageable=new Object();
			 if(querydataorder2orderRow[subGrid.dataRow][childRow][i]!=''&&querydataorder2orderRow[subGrid.dataRow][childRow][i][13]!=undefined){
				manageable.id=querydataorder2orderRow[subGrid.dataRow][childRow][i][13][1];
