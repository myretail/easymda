...
<title></title>
	...
	//??¡è???????????????¨¨?¡§		
	 	 					...
	 	 					<script type="text/javascript" src="../order/TaskSubCrudForSubGrid.js"></script>
			<!--sub Table script    eg:taskRow.js    ??????¨ª???????¡ÀchildRow=0-->
										<script type="text/javascript" >
...
function getDetailTaskForm(inRowIndex){
	if(detailRowsTaskForm[inRowIndex]){
		...
		var h =0;
		if(detailRowsTaskForm[inRowIndex]&&querydataTaskForm[inRowIndex][6]!=undefined &&querydataTaskForm[inRowIndex][6].length>0){
...
var addRowTaskForm = function(){
	gridTaskForm.addRow([false,0,"","",""],gridTaskForm.model.getRowCount());
} 
