...
%>
<html>
<!--????¨¤?????????¡À¨ª???????????¡Àvar dataChild=querydataOrgaPeoples[inRowIndex]-->
<head> 
...
<title></title>
	...
	//
				...
				{ name: '\u8be6\u7ec6', colSpan: 5 ,styles: 'padding: 0; margin: 0;', get: getDetailOrgaPeoples }//value: '<div style="height:0px; background-color: white;"></div>'
			  ]
		  ]
...
function getDetailOrgaPeoples(inRowIndex){
	if(detailRowsOrgaPeoples[inRowIndex]){
		var cell = this;
		setTimeout(function() { buildSubgridOrgaPeoples(inRowIndex, cell); }, 0);  }
		var h =0;
		if(detailRowsOrgaPeoples[inRowIndex]&&querydataOrgaPeoples[inRowIndex][$childRow]!=undefined &&querydataOrgaPeoples[inRowIndex][$childRow].length>0){
...
var addRowOrgaPeoples = function(){
	gridOrgaPeoples.addRow([false,0,"","",""],gridOrgaPeoples.model.getRowCount());
} 
...
function call_funUpdOrgaPeoples() {
       ...
       if(passAll==0){
             if(thisCell[1]==0){//create new row
				...
				querydataOrgaPeoples[i][${childRow}]=[addrow];
				});
	         }else{//modify row
                padd.add('OrgaPeoplesVO',vobject);
...
<table  width="100%">
	...
	<span id="rowCountOrgaPeoples"></span>
		<button dojoType="dijit.form.myButton" onclick="upPageOrgaPeoples();"  id="but6OrgaPeoples" disabled='true' jsId="but6OrgaPeoples" iconClass="upPageIcon" ></button>
		<button dojoType="dijit.form.myButton" onclick="downPageOrgaPeoples();" id="but7OrgaPeoples" disabled='true' jsId="but7OrgaPeoples" iconClass="downPageIcon" ></button>
