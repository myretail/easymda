	...
	*/
	...
	var changeIconClass= function(/*dojo.data.Item*/ item,node){
        ...
        //????????????layout??($member in [org.andromda.cartridges.bpm4struts.metafacades.StrutsManageableEntityAssociationEndLogicImpl[orga], org.andromda.cartridges.bpm4struts.metafacades.StrutsManageableEntityAssociationEndLogicImpl[people]])??if(!${member.many})  ??б┴?????????Float???????бу????????
	                                     
              //begin from 41.4
                  	var querydataorga2orgaPeoplesOrga =  new Array();//???????????
					function getTreeNodes(peopleId,ParentSno,chitem,layerInt,layerSum){//?б┴??layerInt=0
						var pu = new SOAPClientParameters2();
						pu.add('id',peopleId) ;
					  ...
					  //var newitem=popStore.newItem({name:ulo["ns:handleName"] ,type:ulo["ns:handleSno"] ,eId:ulo["ns:id"],haveChild:"0",selectIf:ulo["ns:selected"]}, pInfo);//haveChild=0??????1??б┴??ив????2??б┴??ив??
