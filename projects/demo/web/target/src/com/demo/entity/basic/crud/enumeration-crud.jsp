...
<tiles:insert definition="main.layout">
    ...
    <tiles:put name="title" type="string">
        <bean:message key="enumeration.page.title"/>
    </tiles:put>
    ...
    <tiles:put name="javascript" type="string">
        ...
        //<!--
            ...
            function setAction(crud) { document.forms['manageEnumerationForm'].elements['crud'].value = crud; }
            ...
            function setFields(fieldName,enumerationType,id,form)
            {
                form.elements["fieldName"].value = fieldName;
                form.elements["enumerationType"].value = enumerationType;
    ...
    <tiles:put name="body" type="string">
        ...
        <div>
            <h1><bean:message key="enumeration.page.title"/></h1>
        </div>

        <html:form styleId="manageEnumerationForm" action="/Enumeration/Manage" method="post">
            <input type="hidden" name="crud" value=""/>
            <div id="criteria">
                <c:if test="${!empty manageableForm}">
                    <table>
                        <tr>
                            <td><nobr><bean:message key="enumeration.field.name"/> <div class="important">*</div></nobr></td>
                            <td>
                                <html:text name="manageableForm" property="fieldName" styleClass="criteriaField" styleId="fieldName"/>
                            </td>
                        </tr>
                        <tr>
                            <td><nobr><bean:message key="enumeration.enumeration.type"/> <div class="important">*</div></nobr></td>
                            <td>
                                <html:text name="manageableForm" property="enumerationType" styleClass="criteriaField" styleId="enumerationType"/>
                            </td>
                        </tr>
...
<div id="entitySwitcher">
    <nobr>
        ...
        <select onchange="document.location=this.options[this.selectedIndex].value+'?ref_Enumeration='+this.form.elements['id'].value;">
            ...
            <option selected="selected" value="<html:rewrite page="/Enumeration/Manage.do"/>"><bean:message key="enumeration"/></option>
                ...
                <c:if test="${!empty manageableForm.manageableList}">
                    <display:table name="${manageableForm.manageableList}" id="row" requestURI="${pageContext.request.requestURI}"
                        ...
                        <display:column media="html" sortable="false">
                            <nobr>
                                <input type="radio" name="_copy" onclick="enableUpdate(true);setFields('<formatting:escape language="javascript">${row.fieldName}</formatting:escape>','<formatting:escape language="javascript">${row.enumerationType}</formatting:escape>','<formatting:escape language="javascript">${row.id}</formatting:escape>',this.form);"/>
                                <input type="checkbox" name="selectedRows" value="${row.id}" onclick="verifyEnableDelete(this);"/>
                            </nobr>
                        </display:column>
                        <display:column media="xml csv excel pdf"
                            property="fieldName"
                            titleKey="enumeration.field.name"/>
                        <display:column media="html"
                            headerClass="fieldName" paramId="fieldName" maxLength="36"
                            sortProperty="fieldName" sortable="true"
                            titleKey="enumeration.field.name"><nobr><formatting:escape language="javascript,html">${row.fieldName}</formatting:escape></nobr></display:column>
                        <display:column media="xml csv excel pdf"
                            property="enumerationType"
                            titleKey="enumeration.enumeration.type"/>
                        <display:column media="html"
                            headerClass="enumerationType" paramId="enumerationType" maxLength="36"
                            sortProperty="enumerationType" sortable="true"
                            titleKey="enumeration.enumeration.type"><nobr><formatting:escape language="javascript,html">${row.enumerationType}</formatting:escape></nobr></display:column>
                    </display:table>
                </c:if>
            </div>
        ...
        <div id="pageHelpSection">
            <blockquote>
                <a href="" id="pageHelp" style="display:inline;" onclick="openWindow('<html:rewrite action="/Enumeration/ManageHelp"/>','onlinehelp',true,false,760,540); return false;">
...
<title></title>
	 ...
	 <script type="text/javascript" src="../js/Enumeration.js"></script>
	  ...
	  var pagesize=10;
	  var totalCount=0;
	  var queryvo=new EnumerationVOJs();
	  var oddstyle='';
...
function getVO(queryvo,pagenumber,pagesize,orderBy){
      ...
      SOAPClient.invoke(url,"getEnumeration", pu, false,  function GetEnu_callBack(ul, soapResponse){      
           querydata=new Array();
		   querydata=EnumerationVOtoArray_fun(ul,true);
