...
var layoutSingleRow=[{
 ...
 //fieldÖµÄ¬ÏµË³ò£¬´jsÐµtoArray×ªØµquerydateÈ¡
				 ...
				 cells: [ [
      				  ...
      				  {name: i18nStr.customerName, field: 2,width: 8}
					       				,   {name: i18nStr.custEnum, field: 3,width: 8}
...
function getVO(queryvo,pagenumber,pagesize,orderBy){
      ...
      SOAPClient.invoke(url,"getCustomer", pu, false,  function GetEnu_callBack(ul, soapResponse){      
           querydata=new Array();
		   querydata=dojoCustomer.CustomerVOtoArray_fun(ul,false);
...
function addDivSave(){
	...
	var passAll=0;
										//enumeration Í¨StringÍ¬í£¿
				if(dijit.byId("addcustomerName").validate(true)){
					vobject.customerName=dijit.byId("addcustomerName").getValue();
				}else{
					setErrorClass(dijit.byId("addcustomerName")); passAll=1;
				}
															//enumeration Í¨StringÍ¬í£¿
				if(dijit.byId("addcustEnum").validate(true)){
					vobject.custEnum=dijit.byId("addcustEnum").getValue();
				}else{
					setErrorClass(dijit.byId("addcustEnum")); passAll=1;
				}
...
<body class="tundra">
	<div id="addDiv" style="margin:25px 0px 20px 0px;  height:100px; display:none;" >
		<div  class="titleDiv"  >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font i18n="addNote">add</font></div>
			...
			<td align="left"  >
									<input  id="addcustEnum"  jsId="addcustEnum" class="editerInput"  dojoType="dijit.form.myFilteringSelect" store="dojoCustomer.custEnumStore" required="true" />
							</td>
