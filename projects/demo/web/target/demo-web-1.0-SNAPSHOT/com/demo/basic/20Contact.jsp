...
<title></title>
	...
	//
				...
				{ name: '\u8be6\u7ec6', colSpan: 5 ,styles: 'padding: 0; margin: 0;', get: getDetailContact }//value: '<div style="height:0px; background-color: white;"></div>'
			  ]
		  ]
...
function getDetailContact(inRowIndex){
	if(detailRowsContact[inRowIndex]){
		var cell = this;
		setTimeout(function() { buildSubgridContact(inRowIndex, cell); }, 0);  }
		var h =0;
		if(detailRowsContact[inRowIndex]&&querydataContact[inRowIndex][$childRow]!=undefined &&querydataContact[inRowIndex][$childRow].length>0){
...
var addRowContact = function(){
	gridContact.addRow([false,0,"","",""],gridContact.model.getRowCount());
} 
...
function call_funUpdContact() {
       ...
       if(passAll==0){
             if(thisCell[1]==0){//create new row
				...
				querydataContact[i][${childRow}]=[addrow];
				});
	         }else{//modify row
                padd.add('ContactVO',vobject);
...
<table  width="100%">
	...
	<span id="rowCountContact"></span>
		<button dojoType="dijit.form.myButton" onclick="upPageContact();"  id="but6Contact" disabled='true' jsId="but6Contact" iconClass="upPageIcon" ></button>
		<button dojoType="dijit.form.myButton" onclick="downPageContact();" id="but7Contact" disabled='true' jsId="but7Contact" iconClass="downPageIcon" ></button>
