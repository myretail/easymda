...
//一对多
public class One2ManyActionListener implements ActionListener {
	private static final Logger LOG = Logger
			.getLogger(One2ManyActionListener.class);
	...
	//生成子表相关类		
			...
			if(!ifHaveClass(facade.getName(entityClass) + "VO")){
		      ...
		      //为serviceClass加方法
				...
				/*暂时生成子表service begin*/
				//新建serviceClass,
				Object serviceSubClass = coreFactory.buildClass(facade.getName(entityClass)+ "ManageService", thePackage);
				//显示serviceClass,
				FigClass serviceFig = new FigClass(serviceSubClass, new Rectangle(10,10, 20, 20), new DiagramSettings());
				classDiagram.add(serviceFig);
				serviceFig.setName(facade.getName(entityClass) + "ManageService");
				
				coreFactory.buildDependency(serviceSubClass, entityClass);
				//设置class的属性
			 	Model.getExtensionMechanismsHelper().addCopyStereotype(serviceSubClass,getStereotype(project, "Service") );
		        Model.getExtensionMechanismsHelper().addCopyStereotype(serviceSubClass,getStereotype(project, "WebService") );
				
		        //为serviceClass加方法
		         
		        
		        Object createMethodSub = coreFactory.buildOperation2(serviceSubClass, attrType, "create"+facade.getName(entityClass) );
		        Object updateMethodSub = coreFactory.buildOperation2(serviceSubClass, returnType, "update"+facade.getName(entityClass) );
		        Object delMethodSub = coreFactory.buildOperation2(serviceSubClass, returnType, "delete"+facade.getName(entityClass) );
		        
		        Object queryMethodSub = coreFactory.buildOperation2(serviceSubClass, queryReturnType, "get"+facade.getName(entityClass) );
		        
		        Object parameterCreateSub= coreFactory.buildParameter(createMethodSub, project.findTypeInModel(facade.getName(entityClass)+"VO",thePackage));
		        coreHelper.setName(parameterCreateSub, facade.getName(entityClass));//要小写
		        
		        Object parameterUpdateSub= coreFactory.buildParameter(updateMethodSub, project.findTypeInModel(facade.getName(entityClass)+"VO",thePackage));
		        coreHelper.setName(parameterUpdateSub, facade.getName(entityClass));//要小写
		        
		        Object parameterDelSub= coreFactory.buildParameter(delMethodSub, attrType);
		        coreHelper.setName(parameterDelSub, "id");//要小写
		        
		        Object parameterQuery1Sub= coreFactory.buildParameter(queryMethodSub, project.findTypeInModel(facade.getName(entityClass)+"VO",thePackage));
		        coreHelper.setName(parameterQuery1Sub, facade.getName(entityClass));////要小写
		       
		        Object parameterQuery2Sub= coreFactory.buildParameter(queryMethodSub, intType);
		        coreHelper.setName(parameterQuery2Sub, "pageNumber");
		        Object parameterQuery3Sub= coreFactory.buildParameter(queryMethodSub, intType);
		        coreHelper.setName(parameterQuery3Sub, "pageSize");
		        Object parameterQuery4Sub= coreFactory.buildParameter(queryMethodSub, stringType);
		        coreHelper.setName(parameterQuery4Sub, "orderBy");
		        /*暂时生成子表service end*/
			}
//--------------------------			
